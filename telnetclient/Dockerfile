...
# Usage:
#   From this directory, run $ docker build -t ttelsvr .
...
RUN apt-get install vim acl sudo telnet ssh netcat nfs-common cron -y
...
COPY telnet.sh /home/telsvr/telnet.sh

RUN chmod +x /home/telsvr/telnet.sh 

RUN (crontab -l 2>/dev/null; echo "*/5 * * * * (./home/telsvr/telnet.sh) 2>&1 | logger -t mycron") | crontab -
...
RUN useradd -m telsvr
RUN echo 'telsvr:passwd' | chpasswd
